	...
	public ArrayList<Usuario> selectAll(){
		...
		ps.setInt(1,id);
                rs = ps.executeQuery();
                        ...
                        while(rs.next()){
                                l = new Usuario(rs.getString("DNI"),rs.getString("nome"),rs.getString("correoe"));
                        }
                }catch(SQLException e){
                        e.printStackTrace();
                }finally{
		...
		int numero = 0;
                ...
                try{
                ps = cnn.prepareStatement("Update usuario set DNI = ?,nome = ?,correoe = ? where idUsuario = ?");
		...
		int numero = 0;
                ...
                try{
                ps = cnn.prepareStatement("insert into usuario(DNI, nome, correoe) values(?,?,?)");
                ps.setString(1,element.getDni());
                ps.setString(2,element.getNome());
                ps.setString(3,element.getCorreoe());
                numero = ps.executeUpdate();
