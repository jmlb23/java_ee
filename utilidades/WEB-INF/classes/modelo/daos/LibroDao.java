...
public class LibroDao extends AbstractDao<Libro>{
	...
	public ArrayList<Libro> selectAll(){
		...
		try(PreparedStatement ps = cnn.prepareStatement("Select * from libro");
		...
		try{
		ps = cnn.prepareStatement("Delete from libro where idLibro = ?");
		ps.setInt(1,id);
	...
	//idLibro int primary key AUTO_INCREMENT, ISBN varchar(20), titulo varchar(200), autores varchar(300), ano int
		...
		int numero = 0;
                ...
                try{
                ps = cnn.prepareStatement("Update libro set ISBN = ?,titulo = ?,autores = ?, ano = ? where idLibro = ?");
                ps.setString(1,element.getIsbn());
		...
		int numero = 0;
                ...
                try{
                ps = cnn.prepareStatement("insert into libro(ISBN,titulo,autores,ano) values (?,?,?,?)");
                ps.setString(1,element.getIsbn());
                ps.setString(2,element.getTitulo());
                ps.setString(3,Arrays.toString(element.getAutores()).replaceAll("\\[\\]",""));
                ps.setInt(4,element.getAno());
